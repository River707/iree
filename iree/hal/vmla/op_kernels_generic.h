...
Status ScatterCopy(absl::Span<const T> src_buffer, absl::Span<T> dst_buffer,
  ...
  for (int i = 0; i < src_shape[0]; i++) {
    IREE_RETURN_IF_ERROR(ScatterCopy(src_buffer.subspan(i * src_stride, src_stride),
...
Status ScatterHelper(absl::Span<const T> src_buffer,
  ...
  IREE_RETURN_IF_ERROR(ScatterCopy(src_buffer, dst_buffer.subspan(offset), src_shape,
